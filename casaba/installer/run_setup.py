...
from casaba.modules.common import filtered_hosts
from casaba.version import version_info
...
def _getanswerfilepath():
    ...
    if os.access(p, os.W_OK):
        path = os.path.abspath(os.path.join(p, "casaba-answers-%s.txt" % ts))
...
def _gettmpanswerfilepath():
    ...
    p = os.path.expanduser("~/")
    if os.access(p, os.W_OK):
        path = os.path.abspath(os.path.join(p, "tmp-casaba-answers-%s.txt" % ts))
    ...
    with os.fdopen(fd, "w") as ans_file:
        ...
        for group in controller.getAllGroups():
            for param in group.parameters.itervalues():
                ...
                args = {'comment': comm.encode('utf-8'),
                        'separator': sep,
...
def single_step_install(options, logFile):
    ...
    # We're going to generate the answerfile and run Casaba in a single step
...
def initCmdLineParser():
    ...
    parser.add_option("--install-hosts", help="Install on a set of hosts in a single step. The format should be a comma separated list "
                                              ...
                                              "will also be generated and should be used if Casaba needs to be run a second time ")
