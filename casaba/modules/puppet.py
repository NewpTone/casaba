...
from casaba.installer.exceptions import PuppetError
...
re_notice = re.compile(r"notice: .*Notify\[casaba_info\]"
...
surrogates = [
    ...
    ('Execution of \'.*yum.*install (?P<arg1>.*)\'.*Nothing to do',
        'Package %(arg1)s has not been found in enabled Yum repos.'),
    # Casaba does not cooperate with jruby
    ('jruby', 'Your Puppet installation uses jruby instead of ruby. Package '
              'jruby does not cooperate with Casaba well. You will have to '
              'fix this manually.'),
]
...
def scan_logfile(logpath):
    """
    Returns list of casaba_info/casaba_warn notices parsed from
