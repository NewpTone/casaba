...
Casaba is a utility that uses puppet modules to install OpenStack. It can be used to install each OpenStack service on separate servers, an 'allinone' server, or any combination of these. There are three ways that Casaba can be run.

- casaba
- casaba [options]
- casaba --gen-answer-file=<file> [options] / casaba --answer-file=<file>

The third option allows the user to generate a default answer file, edit the default options, and finally run Casaba a second time using this answer file. This is the easiest way to run Casaba and the one that will be documented here. Optionally, it is possible to set additional command-line options (such as a default password), and those options will be set in the answer file.
...
**CONFIG_SAHARA_INSTALL**
    Specify 'y' to install OpenStack Data Processing (sahara). In case of sahara installation casaba also installs heat.['y', 'n']
...
**CONFIG_NTP_SERVERS**
    Comma-separated list of NTP servers. Leave plain if Casaba should not install ntpd on instances.

**EXCLUDE_SERVERS**
    Comma-separated list of servers to be excluded from the installation. This is helpful if you are running Casaba a second time with the same answer file and do not want Casaba to overwrite these server's configurations. Leave empty if you do not need to exclude any servers.
...
**CONFIG_ENVIRONMENT**
    Puppet environment，["development", "test", "production"]，Default to 'production'.

**CONFIG_DOMAIN_NAME**
    Spcify domainname, format is 'number.regionname.domainname.in'，default to '0.example.domainname.io'.

SSL setup
---------
Casaba supports ability to be get CA certificate and use it to sign all certificates used in OpenStack deployment. Standard workflow for this would be creating Subordinate CA. If Root CA is given to Casaba it is not safe to use it for anything else. If no CA is provided and CONFIG_SSL_CACERT_SELFSIGN is set to 'y' Casaba will use Selfsigned CA.
...
**CONFIG_SSL_CACERT_SELFSIGN**
    Specify 'y' if you want Casaba to pregenerate the CA Certificate.
...
**CONFIG_GLANCE_BACKEND**
    Storage backend for the Image service (controls how the Image service stores disk images). Valid options are: file or swift (Object Storage). The Object Storage service must be enabled to use it as a working backend; otherwise, Casaba falls back to 'file'. ['file', 'swift']
...
**CONFIG_CINDER_VOLUMES_CREATE**
    Specify 'y' to create the Block Storage volumes group. That is, Casaba creates a raw disk image in /var/lib/cinder, and mounts it using a loopback device. This should only be used for testing on a proof-of-concept installation of the Block Storage service (a file-backed volume group is not suitable for production usage). ['y', 'n']
...
**CONFIG_NOVA_MANAGE_FLAVORS**
    Whether or not Casaba should manage a default initial set of Nova flavors. Defaults to 'y'.
...
Log files and generated puppet manifests can be found in the /var/tmp/casaba directory under a directory named by the date in which Casaba was run and a random string (for example, /var/tmp/casaba/20131022-204316-Bf3Ek2). Inside the directory are the openstack-setup.log file and a manifest directory, which contains puppet manifests and a log file for each one.

If debugging information is needed while running casaba, the -d switch will make it write more detailed information about the installation.
...
SOURCE
======
* `casaba      https://github.com/openstack/casaba`
* `puppet modules https://github.com/puppetlabs and https://github.com/openstack`
